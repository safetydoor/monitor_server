...
static void *PyDateTimeToString(JSOBJ _obj, JSONTypeContext *tc, void *outValue, size_t *_outLen)
{
	...
	PyObject* newObj= PyObject_CallMethod(obj, "strftime", "s", "%Y-%m-%d %H:%M:%S", NULL);
	GET_TC(tc)->newObj = newObj;
	*_outLen = PyString_GET_SIZE(newObj);
	return PyString_AS_STRING(newObj);
}

static void *PyDateToString(JSOBJ _obj, JSONTypeContext *tc, void *outValue, size_t *_outLen)
{
	PyObject *obj = (PyObject *) _obj;

	PyObject* newObj= PyObject_CallMethod(obj, "strftime", "s", "%Y-%m-%d", NULL);
	GET_TC(tc)->newObj = newObj;
	*_outLen = PyString_GET_SIZE(newObj);
	return PyString_AS_STRING(newObj);
}
...
void Object_beginTypeContext (PyObject *obj, JSONTypeContext *tc)
{
	...
	{
		PRINTMARK();
		pc->PyTypeToJSON = PyDateToString; tc->type = JT_UTF8;
		return;
	}
	else
	if (obj == Py_None)
