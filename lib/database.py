...
class Mysql(object):
    ...
    def connect(self, dba):
        ...
        """
        self.dba = dba
        self.conn = MySQLdb.connect(host=str(dba['host']),user=str(dba['channel']),passwd=str(dba['passwd']),db=str(dba['db']),unix_socket=str(dba['sock']),port=dba['port'])
...
class Model(dict):
    ...
    @classmethod
    def mgr(cls, ismaster=True, **kargs):
...
class Query(object):
    ...
    def orderbyGBK(self, field, direction='ASC'):
        self.order = 'ORDER BY convert(`%s` using gb2312) %s' % (field, direction)
        return self
    ...
    def iterator(self):     
        for row in self.query():
            obj = self.model.__class__(row,db=self.model.db)
            yield obj
