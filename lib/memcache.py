...
class Client(local):
	...
	In all cases where a key is used, the key can be either:
		...
		example, to keep all of a given channel's objects on the same memcache
		server, so you could use the channel's unique id as the hash value.
	...
	def set(self, key, val, time=0, min_compress_len=0):
		...
		You may prefer, for example, to keep all of a given channel's objects
		on the same memcache server, so you could use the channel's unique
